...
def image_generator(cfg: Config, idea: MemeIdea, caption: str = "") -> str:
    ...
    # Tomson description generated by GPT-4 Vision from the reference image
    tomson = (
        "The main character is always 'Tomson': a Pixar-style 3D animated young man "
        "with a large striped pharaoh headdress with gold and black vertical bands. "
        "His hair is tousled and spiky, protruding wildly from beneath the headdress. "
        "He has large expressive blue eyes that convey sadness and exhaustion. "
        "His face is youthful with exaggerated animated features, a small pointed nose, "
        "and a downturned mouth suggesting melancholy and pensiveness. "
        "He wears a formal modern gray blazer over a simple white shirt. "
        "Pixar 3D animation style, cinematic soft lighting, high detail."
    )
...
Main character in every image:
{tomson}

Rules:
- Tomson must be the main character in every image — use the exact description above
...
- Fit the meme format: for two-panel, describe both panels featuring Tomson; for image macro, describe the scene
...
- Respond with just the prompt string, nothing else"""
    ...
    # Step 2: Generate image with DALL-E 3 (retry with simpler prompt on content filter)
    fallback_prompt = (
        f"Pixar 3D animated young man with a gold-and-black striped pharaoh headdress, "
        f"tousled dark hair, large sad blue eyes, gray blazer, white shirt, "
        f"looking dejected and tired. Scene: {idea.concept[:120]}. "
        f"Cinematic lighting, Pixar style, no text."
    )
    image_url = ""
    for attempt, prompt in enumerate([dalle_prompt, fallback_prompt]):
        try:
            ...
            image_url = (img_resp.data[0].url if img_resp and img_resp.data else "") or ""
            break
        except Exception:
            if attempt == 1:
                raise
